...
module datapath (
    ...
    pc u_pc (.clk(clk), .rst_n(rst_n), .en(~stall_F || pc_src_D), .pc_next(pc_next_F), .pc(pc_F));
   ...
   always @(posedge clk or negedge rst_n) begin
    if (!rst_n || pc_src_D) begin          // ← pc_src_D 추가
        instr_D_reg <= 0; 
        pc_plus4_D  <= 0; 
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || flush_E || pc_src_D) begin
    ...
    data_memory u_data_mem (
        ...
        .switches(switches), .leds_out(leds),
